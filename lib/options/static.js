...
exports.handler = function staticHandler(argv) {
  ...
  if (!pkgData.rwb.static_generator) {
    ...
    if (doIt) {
      ...
      pkgData.rwb = pkgData.rwb || {};
      pkgData.rwb.static_generator = './render-static-page.js';

      // Update package.json
      fs.writeJsonSync(pkgFile, pkgData);

      console.info('Copied render-static-page.js to project folder.');

      process.exit(0);
    } else {
      console.error(`'rwb.static_generator' key is not set in ${pkgFile}.`);
      process.exit(1);
    }
  }
  ...
  webpack(config).run(function(err, stats) {
    ...
    webpackRequire(
      config,
      path.join(projectRoot, pkgData.rwb.static_generator),
      ['fs'],
      {
        console,
        RWB: {
